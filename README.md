...
Game media values by its controls and interactivity, marking a difference from films. Therefore a slight bad input, or bad control clearly correlates towards a bad game, but they could be fixed lately.
Although bad controls don't result in plane crash (or maybe it happened), the player usually gets tortured by them throughout the gameplay session, this means the developer gets a more frustrated player base, more middle quitters / refunds, and less positive reviews
...
6. All enemy NPC now knows the player is right here
